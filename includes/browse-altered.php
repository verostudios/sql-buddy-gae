...
$perPage = (isset($sbconfig) && array_key_exists("RowsPerPage", $sbconfig)) ? $sbconfig['RowsPerPage'] : 30;
...
if ($query) {
	...
	foreach ($querySplit as $q) {
		...
		if ($q != "") {
			if (isset($queryTable)) {
				$totalRows = $conn->tableRowCount($db, $queryTable);
			...
			echo '$q = nil in includes/browser.php';
		}
	}
		
	if (!isset($queryTable)) {
		$totalRows = (int)$dataSql->rowCount();
...
if (isset($queryTable) && $conn->getAdapter() == "sqlite") {
	$structure = $conn->describeTable($db, $queryTable);
	...
	$structureSql = $conn->describeTable($db, $queryTable);
	
	if ($conn->isResultSet($structureSql)) {
		while ($structureRow = $structureSql->fetch(PDO::FETCH_ASSOC)) {	
...
if (isset($dbError)) {
	...
	if (isset($totalRows) && $totalRows > 0) {
		...
		if (isset($primaryKeys) && count($primaryKeys) > 0) {
			...
			while (($dataRow = $dataSql->fetch(PDO::FETCH_ASSOC)) && ($m < $displayLimit)) {
		...
		while (($dataRow = $dataSql->fetch()) && ($m < $displayLimit)) {
