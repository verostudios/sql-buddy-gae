...
<tr>
	...
	if ($conn->getAdapter() == "mysql") {	
		...
		if (isset($_SESSION['SB_LOGIN_USER'])) {
			$message = sprintf(__("You are connected to MySQL %s with the user %s."), $dbVersion, $_SESSION['SB_LOGIN_USER'] . "@" . $sbconfig['ProjectId'] . ':' . $sbconfig['InstanceName']);
			// $conn->getOptionValue("host")
		}
