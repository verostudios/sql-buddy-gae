...
if (isset($_POST)) {
	...
	if (isset($indexType) && isset($indexColumnList) && $indexType && $indexColumnList) {
		...
		$indexQuery = "ALTER TABLE $db.$table ADD ";
...
<td valign="top" width="575">
	...
	if (isset($charsetList) && isset($collationList)) {
		...
		$infoSql = $conn->query("SHOW TABLE STATUS FROM $db LIKE '$table'");
		
		if ($conn->isResultSet($infoSql) == 1) {
		
		$infoRow = $infoSql->fetch(PDO::FETCH_ASSOC);
	...
	try {
		$indexListSQL = $conn->query("SHOW INDEX FROM $db.$table;");
	} catch ( PDOException $e ) {
		echo $e->getMessage();
	}
	...
	if ($conn->isResultSet($indexListSQL)) {
		...
		while ($indexListRow = $indexListSQL->fetch(PDO::FETCH_ASSOC)) {
...
<td valign="top">
	...
	$rowCount = $conn->tableRowCount($db, $table);
