...
if (isset($_SESSION['SB_LOGIN_STRING'])) {
	...
	if ( isset($_GET['db']) ) {
		$connStr = $_SESSION['SB_LOGIN_STRING'];
		$newConStr = '';

		preg_match("/mysql:unix_socket=\/cloudsql\/([a-z\-]+):([a-z0-9\-]+)(;dbname=[a-z0-9\-]+)/i", $connStr, $matches);

		if ( count($matches) > 0 ) {
			$newConStr = 'mysql:unix_socket=/cloudsql/' . $matches[1] . ':' . $matches[2] . ';dbname=' . $_GET['db'];
		} else {
			$newConStr = $connStr . ';dbname=' . $_GET['db'];
		}

		$conn = new SQL($newConStr, $user, $pass);
		$_SESSION['SB_LOGIN_STRING'] = $newConStr;
	} else {
